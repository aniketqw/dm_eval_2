...
def main():
    ...
    CONFIG = {
        "CSV_URL": "https://autogluon.s3.amazonaws.com/datasets/timeseries/monash_cif_2016/train.csv",
        "PARQUET_PATH": "/home/h20250169/study/modelTraining/dm_eval_2/base/withoutSampling/singleDataset/short/CIF_2016/dataset/cif_2016_clean.parquet.gz",
    ...
    logger.info("-"*70)
    logger.info(f"  Paper WQL:  0.013")
    logger.info(f"  Your WQL:   {mean_wql:.6f}")
    logger.info(f"  Difference: {abs(mean_wql - 0.013):.6f} ({abs(mean_wql - 0.013)/0.013*100:.1f}%)")
    logger.info("")
    logger.info(f"  Paper MASE: 0.981")
    logger.info(f"  Your MASE:  {mean_mase:.6f}")
    logger.info(f"  Difference: {abs(mean_mase - 0.981):.6f} ({abs(mean_mase - 0.981)/0.981*100:.1f}%)")
    logger.info("="*70)
