...
def main():
    ...
    logger.info("-"*70)
    logger.info(f"  Paper WQL:  0.097")
    logger.info(f"  Your WQL:   {mean_wql:.6f}")
    logger.info(f"  Difference: {abs(mean_wql - 0.097):.6f} ({abs(mean_wql - 0.097)/0.097*100:.1f}%)")
    logger.info("")
    logger.info(f"  Paper MASE: 0.839")
    logger.info(f"  Your MASE:  {mean_mase:.6f}")
    logger.info(f"  Difference: {abs(mean_mase - 0.839):.6f} ({abs(mean_mase - 0.839)/0.839*100:.1f}%)")
    logger.info("="*70)
