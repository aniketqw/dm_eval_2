...
def process_single_series(pipeline, ctx, fut, idx, progress_tracker):
    ...
    try:
        ...
        print(f"Samples shape after squeeze: {samples_np.shape}")  # Should be (100, 8)
        ...
        print(f"Quantile predictions shape: {quantile_predictions.shape}")  # Should be (8, 9)
        ...
        print(f"Median prediction shape: {median_pred.shape}")  # Should be (8,)
        print(f"Future values shape: {fut.shape}")  # Should be (8,)
        ...
        log.error(f"Shapes - fut: {fut.shape if 'fut' in locals() else 'N/A'}, "
                 f"ctx: {ctx.shape if 'ctx' in locals() else 'N/A'}, "
                 f"samples_np: {samples_np.shape if 'samples_np' in locals() else 'N/A'}")
...
def main():
    ...
    try:
        ...
        # Prepare series data - limit to NUM_SERIES
        ...
        log.info(f"Total series in dataset: {len(ds)}, To process: {len(valid_indices)}")
...
# nohup python3 run_chronos_london_smart_meter.py  > eval_output.log 2>&1 &
# kill %1