...
def main():
    ...
    CONFIG = {
        "CSV_URL": "https://autogluon.s3.amazonaws.com/datasets/timeseries/monash_m1_quarterly/train.csv",
        "PARQUET_PATH": "/home/h20250169/study/modelTraining/dm_eval_2/base/withoutSampling/singleDataset/short/M1_QUARTERLY/dataset/m1_quarterly_clean.parquet.gz",
    ...
    logger.info("-"*70)
    logger.info(f"  Paper WQL:  0.105")
    logger.info(f"  Your WQL:   {mean_wql:.6f}")
    logger.info(f"  Difference: {abs(mean_wql - 0.105):.6f} ({abs(mean_wql - 0.1057)/0.105*100:.1f}%)")
    logger.info("")
    logger.info(f"  Paper MASE: 1.739")
    logger.info(f"  Your MASE:  {mean_mase:.6f}")
    logger.info(f"  Difference: {abs(mean_mase - 1.739):.6f} ({abs(mean_mase - 1.739)/1.739*100:.1f}%)")
    logger.info("="*70)
